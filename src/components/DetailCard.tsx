    ...
    <div>
      <div className="flex p-4">
        ...
        <div className='ml-2 flex flex-col justify-between w-full text-ellipsis'>
          <Link to={`/truyen-tranh/${props.manga.id}`} className='text-lg font-bold mb-1 line-clamp-2 break-all'>{getMangaTitle(props.manga)}</Link>
          ...
          <div className='flex items-center justify-between gap-2'>
            <p className='text-sm text-slate-500 items-center line-clamp-1'>
              ...
              {props.chapter.relationships.find(rela => rela.type === 'scanlation_group')?.attributes?.name || ""}
            </p>
