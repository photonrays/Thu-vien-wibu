    ...
    return (
        <Link to={`/chuong/${chapter.id}`} className="grid sm:grid-cols-chapter-grid grid-cols-2 p-2 bg-slate-100 mb-2 hover:bg-slate-300">
            <div>
                ...
                <span className="flex items-center"><Icon icon='octicon:people-24' className="inline mr-2" /><span className="line-clamp-1">{chapter.relationships.find(rela => rela.type === 'scanlation_group')?.attributes?.name}</span></span>
                <span className="flex items-center sm:hidden"><Icon icon='octicon:person-24' className="inline mr-2" /><span className="line-clamp-1">{chapter.relationships.find(rela => rela.type === 'user')?.attributes?.username}</span></span>
            </div>
            ...
            <div className="sm:block hidden">
                ...
                <span className="flex items-center"><Icon icon='octicon:person-24' className="inline mr-2" />{chapter.relationships.find(rela => rela.type === 'user')?.attributes?.username}</span>
            </div>
            <span className="flex items-center sm:justify-center justify-end sm:hidden"><Icon icon='mdi:clock-outline' className="inline mr-2" />{formatNowDistance(new Date(chapter.attributes?.readableAt)) || ""}</span>
...
export default function Chapter({ volume, chapterList }: { volume?: string, chapterList?: ChapterSchema[] }) {
