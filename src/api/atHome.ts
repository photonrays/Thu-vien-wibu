...
export const getAtHomeServerChapterId = async (chapterId: string): Promise<string[]> => {
    const requestParams: GetAtHomeServerChapterIdRequestOptions = {
        forcePort443: false
    }
    const {data} = await axiosInstance.get<GetAtHomeServerChapterIdResponse>(`at-home/server/${chapterId}`, {
        params: requestParams
    });

    let chapterImgList: string[] = []
    if (data) {
        chapterImgList = data.chapter.data.map((img) => `${data.baseUrl}/data/${data.chapter.hash}/${img}`)
    }
